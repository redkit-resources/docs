...
??? buglink "Слева в окне мира появилась странная хрень"
    ...
    ### Слева в окне мира появилась странная хрень
...
??? buglink "UI: Исправление элемента выпадающего списка"

    ### UI: Исправление элемента выпадающего списка

    После пересборки любого fla файла с выпадающим списком вы обнаружите, что они перестанут работать 
    (не будут открываться/закрываться).

    Связанно это с ошибками в логике action script. Для исправления необходимо заменить содержимое файла по пути:
    
    `workspace\gameplay\gui_new\actionscript\red\game\witcher3\controls\W3ScrollingList.as`

    ??? info "Исправленное содержимое файла (развернуть)"
        ```actionscript
        /***********************************************************************
        /** Base scrolling list
        /***********************************************************************
...
??? buglink "Баг UV развертки при импорте mesh"
    При импорте mesh может возникнуть баг, швы на некоторых частях модели. 
    Даже если ничего не изменилось и вы просто экспортировали и снова импортировали модель в движок.
